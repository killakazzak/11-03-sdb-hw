...
Вопрос: Почему при отправке логов через logstash в elastic не отправляются данные. Хотя при тех же настройках логи nginx отправлялись нормально. 
При отправке через Filebeat -> Logstash данные отправляются корректно.
